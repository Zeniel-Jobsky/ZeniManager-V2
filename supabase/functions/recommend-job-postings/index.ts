...
} from "../_shared/job-postings/filters.ts";
import {
  buildIncruitSearchUrl,
  parseIncruitSearch,
} from "../_shared/job-postings/incruit.ts";
import {
  buildJobKoreaSearchUrl,
  parseJobKoreaSearch,
} from "../_shared/job-postings/jobkorea.ts";
import {
  deduplicateJobPostings,
  normalizeRawPosting,
  splitDesiredJobs,
} from "../_shared/job-postings/normalize.ts";
import { rankJobPostings } from "../_shared/job-postings/ranking.ts";
import {
  buildSaraminSearchUrl,
  parseSaraminSearch,
} from "../_shared/job-postings/saramin.ts";
import {
  JOB_SOURCES,
  JOB_SOURCE_LABELS,
...
type RequestBody = { clientId?: unknown; refresh?: unknown; filters?: unknown };
...
const adapters: SourceAdapter[] = [
  {
    source: "jobkorea",
    buildUrl: buildJobKoreaSearchUrl,
    parse: parseJobKoreaSearch,
  },
  {
    source: "saramin",
    buildUrl: buildSaraminSearchUrl,
    parse: parseSaraminSearch,
  },
  {
    source: "incruit",
    buildUrl: buildIncruitSearchUrl,
    parse: parseIncruitSearch,
  },
];
...
const failureCache = new Map<
  string,
  { expiresAt: number; diagnostics: JobSourceDiagnostic[] }
>();
...
Deno.serve(async request => {
  ...
  try {
    ...
    if (authError || !authData.user) {
      return json(
        { error: "로그인 세션이 만료되었습니다.", code: "UNAUTHORIZED" },
        401
      );
    }
    ...
    if (!UUID_PATTERN.test(clientId)) {
      return json(
        { error: "올바른 내담자 ID가 필요합니다.", code: "INVALID_CLIENT_ID" },
        400
      );
    }
    ...
    if (!clientRecord.found) {
      return json(
        { error: "내담자 정보를 찾을 수 없습니다.", code: "CLIENT_NOT_FOUND" },
        404
      );
    }
    ...
    const desiredJobs = splitDesiredJobs(
      clientRecord.desiredJob,
      MAX_DESIRED_JOBS
    );
    ...
    const cacheKey = `${clientId}:${desiredJobs.join("\u0000")}:${serializeJobFilters(filters)}`;
    ...
    if (!scrapePromise) {
      ...
      scrapePromise = scrapeRecommendations(desiredJobs, filters);
    ...
    try {
      ...
      if (error instanceof AllSourcesFailedError) {
        while (failureCache.size >= MAX_CACHE_ENTRIES) {
          const oldestKey = failureCache.keys().next().value as
            | string
            | undefined;
          if (!oldestKey) break;
      ...
      if (inFlightRequests.get(cacheKey) === scrapePromise)
    ...
    if (error instanceof AllSourcesFailedError) {
      return json(
        {
          error:
            "세 채용사이트에서 공고를 가져오지 못했습니다. 잠시 후 다시 시도해주세요.",
    ...
    console.error(
      "[recommend-job-postings] request failed",
...
async function scrapeRecommendations(
  ...
  const settledBySource = await Promise.all(
      ...
      for (const desiredJob of desiredJobs) {
        ...
        try {
          const html = await fetchSourceHtml(
            adapter.source,
            adapter.buildUrl(desiredJob)
          );
          sourceResults.push({
            status: "fulfilled",
            value: { ...query, raw: adapter.parse(html) },
          });
        } catch (reason) {
  ...
  const filteredBySource = new Map<JobSource, JobPostingRecommendation[]>();
  ...
  settledResults.forEach((result, index) => {
    ...
    const eligible = result.value.raw
      ...
      .filter((posting): posting is JobPostingRecommendation =>
        Boolean(posting)
      );
    const filtered = filterJobPostings(eligible, filters);
    eligibleBySource.get(source)?.push(...eligible);
    filteredBySource.get(source)?.push(...filtered);
    chunksBySource.get(source)?.push(filtered);
  });
  ...
  for (const source of JOB_SOURCES) {
    // Ranking candidates before deduplication preserves the copy whose
    // matchedDesiredJob best explains the title when the same posting appears
    // in more than one desired-job search.
    const rankedCandidates = rankJobPostings(
      interleaveLists(chunksBySource.get(source) ?? []),
      desiredJobs,
      filters
    );
    const deduplicated = deduplicateJobPostings(rankedCandidates);
    const ranked = rankJobPostings(deduplicated, desiredJobs, filters);
    deduplicatedBySource.set(source, ranked);
    normalizedBySource.set(source, ranked.slice(0, MAX_RESULTS_PER_SOURCE));
  }

  const failedQueryCount = settledResults.filter(
    result => result.status === "rejected"
  ).length;
  const interleaved = interleaveBySource(normalizedBySource);
  const results = rankJobPostings(
    deduplicateJobPostings(interleaved),
    desiredJobs,
    filters
  ...
  if (failedQueryCount === queries.length)
    throw new AllSourcesFailedError(diagnostics);
  ...
  return {
    ...
    appliedFilterKey: serializeJobFilters(filters),
    sources: diagnostics,
  };
}
...
function createAuthenticatedClient(authorization: string) {
  ...
  if (!supabaseUrl || !anonKey)
    throw new Error("Supabase Edge Function 환경변수가 설정되지 않았습니다.");
...
async function fetchDesiredJob(
  ...
  return {
    ...
    counselorId:
      typeof data.counselor_id === "string"
        ? data.counselor_id.toLowerCase()
        : null,
  };
}
...
async function callerCanAccessClient(
  ...
  return (
    Number(counselorProfile.role) === 4 ||
    (typeof counselorProfile.id === "string" &&
      counselorProfile.id.toLowerCase() === assignedCounselorId?.toLowerCase())
  );
}

function parseFilters(value: unknown): CanonicalJobFilters {
  try {
    return canonicalizeJobFilters(value);
  } catch (error) {
    if (error instanceof InvalidJobFiltersError) {
      throw new RequestError(400, "INVALID_FILTERS", error.message);
    }
    throw error;
  }
}
...
function interleaveBySource(
  ...
  return interleaveLists(
    JOB_SOURCES.map(source => normalizedBySource.get(source) ?? [])
  );
}
...
function buildDiagnostics(
  ...
  filteredBySource: Map<JobSource, JobPostingRecommendation[]>,
  ...
  return JOB_SOURCES.map(source => {
    ...
    const filtered = filteredBySource.get(source) ?? [];
    const sourceDeduplicated = deduplicatedBySource.get(source) ?? [];
    const returned = finalResults.filter(posting =>
      posting.links.some(link => link.source === source)
    ).length;
    ...
    return {
      ...
      excludedByFilter: Math.max(0, eligible.length - filtered.length),
      excludedDuplicate: Math.max(
        0,
        filtered.length - sourceDeduplicated.length
      ),
      ...(failures.length > 0
...
function getCachedResponse(key: string): JobRecommendationResponse | null {
  ...
  return {
    ...
    sources: response.sources.map(source => ({
      ...source,
      returned: results.filter(posting =>
        posting.links.some(link => link.source === source.source)
      ).length,
    })),
  };
}
...
function setCachedResponse(
  ...
  const baseExpiry =
    Date.now() + (response.partial ? PARTIAL_CACHE_TTL_MS : CACHE_TTL_MS);
...
function enforceRefreshCooldown(key: string): void {
  ...
  if (retryAt > now) {
    ...
    throw new RequestError(
      ...
      `${seconds}초 후에 다시 새로고침해주세요.`
    );
  }
  while (refreshCooldowns.size >= MAX_CACHE_ENTRIES) {
    const oldestKey = refreshCooldowns.keys().next().value as
      | string
      | undefined;
    if (!oldestKey) break;
...
function toPublicSourceError(error: unknown): string {
  ...
  if (/일시적으로 제한|자동 조회 요청을 제한/.test(message))
    return "사이트가 자동 조회를 일시적으로 제한했습니다.";
  if (/구조가 변경/.test(message))
    return "검색 결과 형식이 변경되어 읽지 못했습니다.";
