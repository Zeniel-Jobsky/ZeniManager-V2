...
| 이름 | 역할 | 담당 범위 |
|-------|-------|----------|
| 김욱현 | Project Leader / Backend | 실제 Supabase 스키마·RLS 연결, 상담자 등록 데이터 누락 수정, 대시보드·예약 기능 개선, 관리자 및 취업 성공사례 기능 안정화, 전체 기능 검증 |
| 안지오 | Full Stack | 국민취업지원제도 업무매뉴얼 데이터화, 벡터·하이브리드 검색 구조, manual-chat Edge Function과 근거 표시형 RAG 챗봇 구현  |
| 정현욱 | Full Stack | 잡코리아·사람인·인크루트 스크래핑 오케스트레이션, 검색조건·지역 선택 UI, 마감·중복·엄격조건 필터, 관련도 정렬, JWT/RLS 연동, 자동 테스트와 배포·사용 문서 작성 |
